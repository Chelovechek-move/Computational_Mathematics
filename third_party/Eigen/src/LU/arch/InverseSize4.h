...
struct compute_inverse_size4<Architecture::Target, float, MatrixType, ResultType>
{
  ...
  static void run(const MatrixType &mat, ResultType &result)
  {
    ...
    // DC = D#*C
    CD = pmul(vec4f_swizzle2(D, D, 3, 3, 0, 0), C);
    CD = psub(CD, pmul(vec4f_swizzle2(D, D, 1, 1, 2, 2), vec4f_swizzle2(C, C, 2, 3, 0, 1)));
    ...
    d = pmul(vec4f_swizzle2(CD, CD, 0, 2, 1, 3), AB);
    ...
    // iA = A*|D| - B*D#*C
    iA = pmul(vec4f_swizzle2(B, B, 0, 0, 2, 2), vec4f_movelh(CD, CD));
    iA = padd(iA, pmul(vec4f_swizzle2(B, B, 1, 1, 3, 3), vec4f_movehl(CD, CD)));
    ...
    iC = pmul(A, vec4f_swizzle2(CD, CD, 3, 0, 3, 0));
    iC = psub(iC, pmul(vec4f_swizzle2(A, A, 1, 0, 3, 2), vec4f_swizzle2(CD, CD, 2, 1, 2, 1)));
